...
def get_adaptive_binary_mask(logit): ## VOC COCO
...
# def get_adaptive_binary_mask(logit):  ## Cityscapes
#     conf = torch.softmax(logit, dim=1)
#     # import ipdb
#     # ipdb.set_trace()
#     max_value, _ = torch.max(conf.reshape(logit.shape[0], logit.shape[1], -1), dim=2)
#     # print("============================",max_value.shape,"====================================")
#     # print(max_value)
#     new_max = torch.where(max_value > 0.3, max_value * 0.5, max_value)
#     thresh = new_max.unsqueeze(-1).unsqueeze(-1)
#     binary_mask = (conf > thresh)
#     result = torch.sum(binary_mask, dim=1)*0+1
#     return result#, max_value