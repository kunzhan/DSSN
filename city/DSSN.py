...
from util.utils import count_params, init_log, AverageMeter, update_teacher
...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            preds_ema = model_tea(img_u_w)
            conf_u_w_tea = preds_ema.softmax(dim=1).max(dim=1)[0]
            mask_u_w_tea = preds_ema.argmax(dim=1)
            ...
            if epoch < 5:
                loss = (loss_x + loss_u_w_fp * 0.25 + loss_u_w_Fp * 0.25) / 2.0 + \
                            (loss_u_s_mse + loss_u_fp_mse) * 0.005
                keep_rate = 0
            else:
                loss = (loss_x + loss_u_s1 * 0.25 + loss_u_s2 * 0.25 + loss_u_w_fp * 0.25 + loss_u_w_Fp * 0.25) / 2.0 + \
                            (loss_u_s_mse + loss_u_fp_mse) * 0.005
                keep_rate = 0.996
        ...
        if rank == 0:
            ...
            print('alpha: {:.6f}, number: {}\n'.format(keep_rate, int(1/(1-keep_rate))))
